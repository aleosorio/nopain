...
Tibble de una sesión de session_data, obtenida vía API get_form_data, de Zingtree (se le suma columna ID, "Session", al principio):
...
Función Crit_Mov, que crea Tibble de movimientos críticos (varios a una), a partir de un registro de session_data:
```{r}
...
Función Sympt_Diagn_Sol.  Idem Crit_MOv, para movimientos críticos de la sesión (1 o más):
```{r}
...
Función TG: Idem anteriores, para gestos técnicos.  Última variable (TGs) es en base a listas de Tibbles (1 a varios gestos técnicos e intensidades).  Se construye en base a vectores de gestos (detalle por sesión) y zonas (ramas del árbol por las que se pasea durante una sesión):

1. TG - gestos: Vector de nombres de session_data, que contengan "TG" y terminen con un número (ej: TG_1_1_1)
```{r}
gestos <- session_data %>%
  colnames(.) %>%
  str_subset(., "TG") %>%
...
2. TG - zonas: Vector con patrones de gestos, en base a primeros 5 digitos iguales (ej: TG1_1)
```{r}
zonas <- gestos %>%
...
3. TG - Tibble: Idem anteriores funciones, para gestos técnicos, mediante vectores anidados en base a zonas (ej de un vector: TG1_1_1, TG1_1_2, etc.).
```{r}
...
Tibble final de sesión única, en base a inner_join de tablas crit_mov, sympt_diagn_sol y TG:
```{r}
