...
if (numcritmov > 0) {
  ...
  crit_mov <- mutate(session_data, CritMov = "", CritMov_Value = "")
}
```
...
if (numzonas > 0) {
  ...
  TG <- tibble(Session = session_data$Session[[1]], Data = list(tibble(TG = "", TG_Val = "")))
}
```

Tibble final, en base a una sola sesión: PENDIENTE
```{r}
