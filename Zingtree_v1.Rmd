...
Función de extracción de sesiones (agent_sessions) para un período (entre dateini y datefin):
```{r, message = FALSE}
...
Tabla con detalle de sesiones para el período dateini - datefin:
```{r}
...
#Tibble del período (, en base a la (unión de los tibble_session del loop anterior), limpio de NAs y variables 3 en adelante, alfabéticamente:
tibble_total <- bind_rows(tibble_total, tibble_session) %>%
  replace(., is.na(.), "") %>%
  select(., order(colnames(.))) %>%
...
```
PENDIENTE ELIMINAR PRIMERA FILA (VER DE QUE SE CONSTRUYA SIN NULL EN COLUMNA TGs, PARA LUEGO ELIMINAR FILAS CON 100% VALORES = "")
