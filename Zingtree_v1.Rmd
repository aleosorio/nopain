...
#### I.b.i. Función de extracción para un sesion_id
...
#### I.b.ii. Extracción de primera sesión de "agent_sessions" mediante función "get_session_data"

- **Variable**: agent_sessions$session_id[1]
- **output**: Lista con un registro

```{r}
get_form_data(agent_sessions$session_id[1]) %>%
  as_tibble(.) %>%
  str(.)
```
